...
class FloorPlanScene: SKScene {
    ...
    init(capturedRoom: CapturedRoom) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(captureTexture), name: NSNotification.Name("captureSKTexture"), object: nil)
    ...
    @objc func captureTexture() {
        let texture = scene?.view!.texture(from: scene!)
        let cgImage = texture?.cgImage();
        let image = UIImage(cgImage: cgImage!)
        if let data = image.pngData()  {
            let destinationFolderURL = FileManager.default.temporaryDirectory.appending(path: "Export")
            let destinationURL = destinationFolderURL.appending(path: "exportedScene.png")
            do {
